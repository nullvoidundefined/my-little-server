...
REST API for tracking jobs, recruiters, and recruiting firms. Built with Express 5, TypeScript, and PostgreSQL.

**Data model:** Every row in jobs, recruiters, and recruiting_firms belongs to the authenticated user via a `user_id` foreign key. All queries are scoped by it, so users only see their own data.
...
### Recruiters (requires auth)

| Method   | Path              | Description                                                                                             |
| -------- | ----------------- | ------------------------------------------------------------------------------------------------------- |
| `GET`    | `/recruiters`     | List recruiters; returns `{ data, meta: { total, limit, offset } }`                                     |
| `GET`    | `/recruiters/:id` | Get one recruiter                                                                                       |
| `POST`   | `/recruiters`     | Create recruiter (body: `name`, optional `email`, `phone`, `title`, `linkedin_url`, `firm_id`, `notes`) |
| `PATCH`  | `/recruiters/:id` | Update recruiter (partial body)                                                                         |
| `DELETE` | `/recruiters/:id` | Delete recruiter                                                                                        |

### Recruiting firms (requires auth)

| Method   | Path                    | Description                                                             |
| -------- | ----------------------- | ----------------------------------------------------------------------- |
| `GET`    | `/recruiting-firms`     | List firms; returns `{ data, meta: { total, limit, offset } }`          |
| `GET`    | `/recruiting-firms/:id` | Get one firm                                                            |
| `POST`   | `/recruiting-firms`     | Create firm (body: `name`, optional `website`, `linkedin_url`, `notes`) |
| `PATCH`  | `/recruiting-firms/:id` | Update firm (partial body)                                              |
...
- Use a **separate test database** (e.g. a Neon branch or a local `my_little_server_test`). Set `DATABASE_URL` so integration tests never touch dev/prod data.
